...
def parse_args():
    ...
    g_hp.add_argument("--lambda-coef", type=float, default=None, help="FLF λ coefficient.")
    g_hp.add_argument("--sigma-coef", type=float, default=None, help="FLF σ coefficient.")
    ...
    inp = InputConfig(data=pick("data"), sep=_parse_sep(pick("sep")), columns=pick("columns"))
    hp = HyperParams(
...
def make_dataset(values: np.ndarray, window: int):
    x_list, y_list = [], []
    for idx in range(len(values) - window):
        x_list.append(values[idx : idx + window, :])
        y_list.append(values[idx + window, :])
...
def build_model(hp: HyperParams) -> Sequential:
    model = Sequential(
        [
            InputLayer(input_shape=(hp.window, 4)),
            Bidirectional(
                LSTM(
                    units=hp.units,
...
def main():
    ...
    if len(values) <= hp.window + 1:
        raise ValueError(f"Not enough rows ({len(values)}) for window={hp.window}.")

    split_idx = int(len(values) * hp.split)
