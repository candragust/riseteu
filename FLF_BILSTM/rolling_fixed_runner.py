...
if __package__:
    from .bilstm_flf_experiment import load_data, find_ohlc
else:
    from bilstm_flf_experiment import load_data, find_ohlc
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run rolling (fixed-window) evaluation using bilstm_flf_experiment.py."
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument("--train-months", type=int, default=6, help="Length of training window in months.")
    parser.add_argument("--test-months", type=int, default=6, help="Length of testing window in months.")
    parser.add_argument(
        "--shift-months",
        type=int,
        default=None,
    ...
    parser.add_argument(
        "--base-config",
        type=str,
        default=str(ROOT / "final_config.json"),
        help="Config JSON used as hyperparameter baseline.",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default=str(ROOT / "results" / "rolling_fixed"),
...
def build_folds(
    ...
    while True:
        ...
        fold_df = pd.concat([df.loc[train_mask, ["open", "high", "low", "close"]],
                             df.loc[test_mask, ["open", "high", "low", "close"]]], ignore_index=True)
        total = len(fold_df)
        ...
        # advance by test window or expand
        if mode == "fixed":
...
def run_fold(fold: FoldInfo, fold_df: pd.DataFrame, base_cfg: Path, out_dir: Path):
    ...
    print(f"[FOLD {fold.index:02d}] Train {fold.train_start.date()} -> {fold.train_end.date()} | "
...
def main():
    ...
    if args.last_n_folds is not None and args.last_n_folds > 0:
        folds_raw = folds_raw[-args.last_n_folds :]
    if not folds_raw:
        raise RuntimeError("No valid folds produced. Try reducing min-test-samples or adjusting window sizes.")
