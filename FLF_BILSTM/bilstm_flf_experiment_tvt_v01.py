...
from tensorflow.keras.layers import Activation, Bidirectional, Dense, InputLayer, LSTM
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="EURUSD BiLSTM experiment with Forex Loss Function (FLF)."
    )
    ...
    g_in.add_argument("--train-data", type=str, default=None, help="Path to train_core CSV/TSV.")
    g_in.add_argument("--validation-data", type=str, default=None, help="Path to validation CSV/TSV.")
    g_in.add_argument("--test-data", type=str, default=None, help="Path to test CSV/TSV.")
    ...
    g_hp.add_argument("--units", type=int, default=None, help="LSTM units per direction.")
    ...
    inp = InputConfig(
        ...
        validation_data=pick("validation_data"),
...
def apply_standardize(values: np.ndarray, mean: np.ndarray, std: np.ndarray):
    return (values - mean) / std


def prepare_feature_target_arrays(df_raw: pd.DataFrame, columns: Optional[str], feature_columns: Optional[str]):
    ohlc_df = find_ohlc(df_raw, columns)

    feature_cols = []
    extra_df = None
    if feature_columns:
        feature_cols = [c.strip() for c in re.split(r"[,;]", feature_columns) if c.strip()]
    ...
    return feature_values, target_values, len(feature_names)
...
def train_and_predict(model: Sequential, X_train, y_train, X_val, y_val, X_pred, hp: HyperParams):
...
def main():
    ...
    use_explicit_tvt = bool(inp.train_data and inp.validation_data and inp.test_data)

    if use_explicit_tvt:
        train_raw = load_data(resolve_data_path(inp.train_data), inp.sep)
        val_raw = load_data(resolve_data_path(inp.validation_data), inp.sep)
        test_raw = load_data(resolve_data_path(inp.test_data), inp.sep)

        train_feat, train_tgt, feature_dim = prepare_feature_target_arrays(train_raw, inp.columns, inp.feature_columns)
        val_feat, val_tgt, _ = prepare_feature_target_arrays(val_raw, inp.columns, inp.feature_columns)
        test_feat, test_tgt, _ = prepare_feature_target_arrays(test_raw, inp.columns, inp.feature_columns)

        if len(train_feat) <= hp.window + 1:
            raise ValueError(f"Not enough training rows ({len(train_feat)}) for window={hp.window}.")
        if len(val_feat) < 1 or len(test_feat) < 1:
            raise ValueError("Validation and test segments must each contain at least one row.")

        train_feat_std, _, feat_mean, feat_std = standardize(train_feat, train_feat)
        train_tgt_std, _, tgt_mean, tgt_std = standardize(train_tgt, train_tgt)
        val_feat_std = apply_standardize(val_feat, feat_mean, feat_std)
        val_tgt_std = apply_standardize(val_tgt, tgt_mean, tgt_std)
        test_feat_std = apply_standardize(test_feat, feat_mean, feat_std)
        test_tgt_std = apply_standardize(test_tgt, tgt_mean, tgt_std)

        X_train, y_train = make_dataset(train_feat_std, train_tgt_std, hp.window)

        val_feat_src = np.concatenate([train_feat_std[-hp.window :], val_feat_std], axis=0)
        val_tgt_src = np.concatenate([train_tgt_std[-hp.window :], val_tgt_std], axis=0)
        X_val, y_val = make_dataset(val_feat_src, val_tgt_src, hp.window)

        pretest_feat_std = np.concatenate([train_feat_std, val_feat_std], axis=0)
        pretest_tgt_std = np.concatenate([train_tgt_std, val_tgt_std], axis=0)
        test_feat_src = np.concatenate([pretest_feat_std[-hp.window :], test_feat_std], axis=0)
        test_tgt_src = np.concatenate([pretest_tgt_std[-hp.window :], test_tgt_std], axis=0)
        X_test, y_test = make_dataset(test_feat_src, test_tgt_src, hp.window)
        ...
        feature_values, target_values, feature_dim = prepare_feature_target_arrays(df_raw, inp.columns, inp.feature_columns)
        ...
        print(f"Train shapes: X={X_train.shape}, y={y_train.shape}")
        print(f"Val/Test shapes: X={X_val.shape}, y={y_val.shape}")

    model = build_model(hp, feature_dim)
    model.summary(print_fn=lambda line: print("  " + line))

    preds_std, history = train_and_predict(model, X_train, y_train, X_val, y_val, X_test, hp)

    preds = preds_std * tgt_std + tgt_mean
    targets = y_test * tgt_std + tgt_mean
