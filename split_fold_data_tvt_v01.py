...
PROFILE_DEFAULTS = {
    "h4": {
        ...
        "validation_months": 1,
        "test_months": 1,
        "out_dir": str(ROOT / "results" / "splits" / "tvt_v01" / "h4"),
    },
    "d1": {
        ...
        "validation_months": 3,
        "test_months": 1,
        "out_dir": str(ROOT / "results" / "splits" / "tvt_v01" / "d1"),
    },
}
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory. Defaults to results/splits/tvt_v01/<profile>.",
    )
...
def write_fold_outputs(out_dir: Path, profile: str, folds: list[dict]):
    ...
    if summary_rows:
        last5 = summary_rows[-5:]
        last10 = summary_rows[-10:]
        last12 = summary_rows[-12:]
        pd.DataFrame(last5).to_csv(out_dir / "fold_schedule_last5.csv", index=False)
        pd.DataFrame(last10).to_csv(out_dir / "fold_schedule_last10.csv", index=False)
        pd.DataFrame(last12).to_csv(out_dir / "fold_schedule_last12.csv", index=False)
        pd.DataFrame(last10).to_csv(out_dir / "fold_schedule_tuning.csv", index=False)
        pd.DataFrame(last5).to_csv(out_dir / "fold_schedule_evaluation.csv", index=False)

        scope = {
            "profile": profile,
            "total_folds": len(summary_rows),
            "tuning_policy": {
                "description": "Use the last 10 complete folds for hyperparameter tuning.",
                "folds": [int(row["fold"]) for row in last10],
            },
            "evaluation_policy": {
                "description": "Use the last 5 complete folds for final comparative evaluation.",
                "folds": [int(row["fold"]) for row in last5],
            },
        }
        (out_dir / "fold_scope_tvt_v01.json").write_text(json.dumps(scope, indent=2), encoding="utf-8")
