...
def parse_args():
    parser = argparse.ArgumentParser(description="EUR/USD 1D FLF-LSTM tuning pipeline runner.")
    parser.add_argument(
        "--base-config",
        default=str(ROOT / "configs" / "d1_ohlc" / "lstm_flf_config_d1_ohlc_base_v01.json"),
...
def run_job(job_name: str, overrides: Dict, base_cfg: Path, out_dir: Path):
    ...
    cmd = [
        sys.executable,
        str(ROOT / "lstm_flf_experiment.py"),
        "--config",
        str(base_cfg),
        "--out",
        str(preds_path),
    ...
    run(cmd, check=True, cwd=str(PROJECT_ROOT))
...
def main():
    ...
    html = [
        "<html><head><meta charset='UTF-8'><title>EUR/USD 1D FLF-LSTM Pipeline Summary</title>"
        ...
        "</head><body><h1>EUR/USD 1D FLF-LSTM Pipeline Summary (Stage 1-7)</h1>"
    ]
    html.extend(html_sections)
    html.append("</body></html>")
    out_file = out_dir / "eurusd_lstm_d1_pipeline_summary_v01.html"
    out_dir.mkdir(parents=True, exist_ok=True)
