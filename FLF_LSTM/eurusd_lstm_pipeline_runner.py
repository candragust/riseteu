...
def parse_args():
    parser = argparse.ArgumentParser(description="EURUSD FLF-LSTM tuning pipeline runner.")
    parser.add_argument(
        "--base-config",
        default=str(ROOT / "lstm_flf_config_wf72_test1_latest.json"),
        help="Baseline config JSON.",
    )
    parser.add_argument(
        "--out-dir",
        default=str(ROOT / "results" / "lstm_pipeline_wf72_test1"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Run stages up to this number only (e.g. 1 for Stage 1 only).",
    )
...
def main():
    ...
    stages: List[Dict] = [
        {
            ...
            "base": "lstm_s1",
            "jobs": [
                {"name": "lstm_s1_w10", "params": {"window": 10}},
                {"name": "lstm_s1_w12", "params": {"window": 12}},
                {"name": "lstm_s1_w14", "params": {"window": 14}},
            ],
        },
        {
            "id": "stage2",
            ...
            "base": "lstm_s2",
            "jobs": [
                {"name": "lstm_s2_h224", "params": {"units": 224}},
                {"name": "lstm_s2_h256", "params": {"units": 256}},
            ],
        },
        {
            "id": "stage3",
            ...
            "base": "lstm_s3",
            "jobs": [
                {"name": "lstm_s3_relu_lr5e4", "params": {"activation": "relu", "lr": 5e-4}},
                {"name": "lstm_s3_relu_lr7e4", "params": {"activation": "relu", "lr": 7e-4}},
                {"name": "lstm_s3_relu_lr9e4", "params": {"activation": "relu", "lr": 9e-4}},
            ],
        },
        {
            "id": "stage4",
            ...
            "base": "lstm_s4",
            "jobs": [
                {"name": "lstm_s4_lam08_sig015", "params": {"lambda_coef": 0.8, "sigma_coef": 0.15}},
                {"name": "lstm_s4_lam09_sig01", "params": {"lambda_coef": 0.9, "sigma_coef": 0.1}},
                {"name": "lstm_s4_lam08_sig01", "params": {"lambda_coef": 0.8, "sigma_coef": 0.1}},
            ],
        },
        {
            "id": "stage5",
            ...
            "base": "lstm_s5",
            "jobs": [
                {"name": "lstm_s5_batch96", "params": {"batch": 96}},
                {"name": "lstm_s5_batch128", "params": {"batch": 128}},
            ],
        },
        {
            "id": "stage6",
            ...
            "base": "lstm_s6",
            "jobs": [
                {"name": "lstm_s6_e40", "params": {"epochs": 40}},
                {"name": "lstm_s6_e50", "params": {"epochs": 50}},
                {"name": "lstm_s6_e60", "params": {"epochs": 60}},
            ],
        },
    ]
    ...
    html = [
        "<html><head><meta charset='UTF-8'><title>EURUSD FLF-LSTM Pipeline Summary</title>"
        ...
        "</head><body><h1>EURUSD FLF-LSTM Pipeline Summary (72m Train / 1m Test, Latest Fold)</h1>"
    ]
    html.extend(html_sections)
    html.append("</body></html>")
    out_file = out_dir / "eurusd_lstm_pipeline_summary.html"
    out_dir.mkdir(parents=True, exist_ok=True)
