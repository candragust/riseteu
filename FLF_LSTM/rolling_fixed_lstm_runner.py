...
from lstm_flf_experiment import load_data, find_ohlc
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run rolling (fixed-window) evaluation using lstm_flf_experiment.py."
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--base-config",
        type=str,
        default=str(ROOT / "lstm_flf_config.json"),
        help="Config JSON used as hyperparameter baseline.",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default=str(ROOT / "results" / "rolling_train72_test1"),
