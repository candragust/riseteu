...
DEFAULT_SCHEDULE = PROJECT_ROOT / "results" / "splits" / "tvt_v01" / "h4" / "fold_schedule_evaluation.csv"
DEFAULT_FOLD_ROOT = PROJECT_ROOT / "results" / "splits" / "tvt_v01" / "h4"
DEFAULT_BASE_CONFIG = ROOT / "lstm_flf_config_wf72_test1_best.json"
DEFAULT_OUT_DIR = ROOT / "results" / "tvt_v01" / "h4_evaluation_last5"
EXPERIMENT_SCRIPT = ROOT / "lstm_flf_experiment_tvt_v01.py"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run LSTM evaluation on pre-split train/validation/test folds."
    )
    parser.add_argument(
        "--schedule",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Base JSON config for the LSTM experiment.",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
